...
export function useChats() {
  ...
  const updateChat = (chatId: number, updates: Partial<Pick<Chat, "name" | "avatar" | "description" | "category" | "isArchived" | "isPinned" | "showOnline" | "readReceipts" | "readDelayMinutes" | "background">>) => {
    setChats(prev =>
      prev.map(chat =>
        chat.id === chatId
          ? { ...chat, ...updates }
