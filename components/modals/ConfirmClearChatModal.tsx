...
type ConfirmClearChatModalProps = {
  isOpen: boolean
  chatName?: string
  onCancel: () => void
  onConfirm: () => void
...
export default function ConfirmClearChatModal({
  isOpen,
  chatName,
  onCancel,
  onConfirm,
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4">
      <div className="bg-[#0b1014] text-white w-full max-w-md rounded-xl p-6">
        <h2 className="text-lg font-medium mb-2">Vaciar chat</h2>
        <p className="text-gray-300 mb-6">
          {`¿Desea vaciar el chat${chatName ? ` "${chatName}"` : ""}? Se borrarán todos los mensajes. Esta acción no se puede deshacer.`}
        </p>
